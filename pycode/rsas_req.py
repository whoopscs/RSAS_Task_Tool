...
class RSAS_Requests:
    ...
    def RSAS_Login(self, scanner_url, username, password):
        ...
        if csrfmiddlewaretoken:
            print('[+] 获取csfrmiddlewaretoken成功，正在重构请求头并发送payload...')
        else:
            print('[!] 获取csrfmiddlewaretoken失败！')
            exit(0)
...
class Start_Host_Scan_Working(QThread):
    ...
    def run(self):
        ...
        print("[+] 主机扫描的csrfmiddlewaretoken获取成功！")
        ...
        for _task in self.task_list_host:
            ...
            if 'suc' in host_resp.text:
                ...
                print(f"新建扫描任务成功，任务编号：{host_resp.text.split(':')[2]} ，任务名称：{task_name}")
            else:
                self.start_host_return.emit(f'第{i}个任务下达任务失败...(详情见log.txt)')
                print(f"新建任务失败：{task_name} ，报错信息：{json.loads(host_resp.text[1:-1])}")
                with open('log.txt', 'a') as content:
...
class Start_Web_Scan_Working(QThread):
    ...
    def run(self):
        ...
        print("[+] Web扫描的csrfmiddlewaretoken获取成功！")
        ...
        for _task in self.task_list_web:
            ...
            if 'suc' in web_resp.text:
                ...
                print(f"新建扫描任务成功，任务编号：{web_resp.text.split(':')[2]} ，任务名称：{task_name}")
            else:
                self.start_web_return.emit(f'第{i}个任务下达任务失败...(详情见log.txt)')
                print(f"新建任务失败：{task_name} ，报错信息：{json.loads(web_resp.text[1:-1])}")
                with open('log.txt', 'a') as content:
                    content.write(log_time)
                    content.write(f'     新建任务失败：{task_name}     报错信息：')
...
class RSAS_Status(QThread):
    ...
    def run(self):
        ...
        while True:
            ...
            servertime = json.loads(json_response).get('time')

            self.log_return.emit(f'{nowtask_num}|{waittask_num}|{servertime}')
            time.sleep(10)