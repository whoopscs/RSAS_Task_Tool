...
class main_window(QWidget, Ui_main):
    def __init__(self, scanner_url, username, passwd, parent=None):
        ...
        self.center_widget_layout = QGridLayout()
    ...
    def Load_Task_Name_Host(self):
        ...
        for file in os.listdir(HostAssets_dir):
            if os.path.splitext(file)[1] == '.txt':
                ...
                self.newtask_ui.Task_name_Host_textEdit.append(file[0:-4]) # str[0:-1]为切片，意思是从前面开始截取到后面-1为止
    ...
    def web_scan_tab(self):
        ...
        self.newtask_ui.Scan_Range_comboBox.setCurrentIndex(1)
        # 设置并发线程数默认值
        ...
        self.newtask_ui.Dir_level_comboBox.setCurrentIndex(1)
        # 设置目录猜测深度默认值
        self.newtask_ui.Dir_limit_lineEdit.setText("3")

        self.newtask_ui.Load_Task_Name_Web_pushButton.clicked.connect(self.Load_Task_Name_Web)
    ...
    def set_task_view(self):
        ...
        task_list_tableView.setSelectionBehavior(QHeaderView.SelectRows)
        task_list_tableView.setAlternatingRowColors(True)
        task_list_tableView.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
